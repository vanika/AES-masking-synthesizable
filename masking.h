...
void calcInvMixColmask(uint8_t mask[10]);
void remask(state_t * s, uint8_t m1, uint8_t m2, uint8_t m3, uint8_t m4, uint8_t m5, uint8_t m6, uint8_t m7, uint8_t m8);
void calcSboxMasked(uint8_t mask[10]);
void calcrSboxMasked(uint8_t mask[10]);
void InitMaskingDecrypt(state_t * s,const uint8_t *RoundKey);
void InitMaskingEncrypt(state_t * s, uint8_t *RoundKey);
...
void InitMaskingEncrypt(state_t * s, uint8_t *RoundKey)
{
    ...
    // since the programmer keep using the reference to the old key
    remask(s, mask[6], mask[7], mask[8], mask[9], 0, 0, 0, 0);
    for(uint8_t i = 0; i < 10; i++){
        saved_mask[i] = mask[i];
    }

    for(uint8_t i = 0; i < AES_keyExpSize; i++){
        RoundKey[i] = RoundKeyMasked[i];
    }
}


// mask[0], mask[1], mask[2], mask[3], mask[5], mask[5], mask[5], mask[5]

//uint8_t m1 = 0, uint8_t m2 = 1, uint8_t m3 = 2, uint8_t m4 = 3, uint8_t m5 = 5, uint8_t m6 = 5, uint8_t m7 = 5, uint8_t m8 = 5
void remaskLoop(state_t *s){

    for (int i = 0; i < 4; i++)
    {
        (*s)[i][0] = (*s)[i][0] ^ (saved_mask[0] ^ saved_mask[5]);
        (*s)[i][1] = (*s)[i][1] ^ (saved_mask[1] ^ saved_mask[5]);
        (*s)[i][2] = (*s)[i][2] ^ (saved_mask[2] ^ saved_mask[5]);
        (*s)[i][3] = (*s)[i][3] ^ (saved_mask[3] ^ saved_mask[5]);
    }
}
